  ...
  for (int i = 0; i < roundedx+1; i++) {
    for (int j = 0; j < roundedy+1; j++) {
      if (red(logo.get(gap * i,gap * j)) > 100) grid.add(new Grid(gap*i, gap*j, dotSize, gridSpring));
       if (red(logo.get(gap * i + (gap/4),gap * j)) < 100) grid.add(new Grid(gap*i + (gap/4), gap * j, dotSize, gridSpring));
      else if (red(logo.get(gap*i, gap * j + (gap/4))) == 0) grid.add(new Grid(gap*i, gap * j + (gap/4), dotSize, gridSpring));
      else if (red(logo.get(gap*i + (gap/2), gap * j)) == 0) grid.add(new Grid(gap*i + (gap/2), gap * j, dotSize, gridSpring));
      else if (red(logo.get(gap*i, gap * j + (gap/2))) == 0) grid.add(new Grid(gap*i, gap * j + (gap/2), dotSize, gridSpring));
      else if (red(logo.get(gap*i + (gap/4*3), gap * j)) == 0) grid.add(new Grid(gap*i + (gap/4*3), gap * j, dotSize, gridSpring));
      else if (red(logo.get(gap*i, gap * j + (gap/4*3))) == 0) grid.add(new Grid(gap*i, gap * j + (gap/4*3), dotSize, gridSpring));
    }
  }
...
void draw() {
  ...
  if(skeletonArray.size() != gravity.size() / 3)
  {
    ...
    while(skeletonArray.size() > gravity.size() / 3)
      for(int i = 0; i < 3; i++) gravity.add( new Point(new PVector(width/4, height/4), 20, 10000) );
    while(skeletonArray.size() > gravity.size() / 3)
